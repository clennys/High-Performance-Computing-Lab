...
int main(int argc, char **argv) {
  ...
  if (mpi_rank == 0) {
    ...
    for (int proc = 1; proc < mpi_size; proc++) {
      ...
      // data)
      MPI_Recv(c, d.nx * d.ny, MPI_INT, proc, 0, p.comm, MPI_STATUS_IGNORE);
  ...
  // TODO: uncomment after you implement createPartition(int mpi_rank, int
  // mpi_size)
